#--------------------------------------
# This module:
#   Creates a set of common functions - tryin to match users of osx and linux (look-n-feel compatibility)
...
#
#     -- various tools are needed to be used for these constructions:
#        ruby
#        npm
#        powershell
#        curl
...
function Unzip-File {
    ...
    .PARAMETER ForceCOM
        Switch parameter to force the use of COM for the extraction even if the .NET Framework 4.6 is present.
    ...
    .NOTES
       ...
       This function first checks to see if the .NET Framework 4.6 is installed and uses it for the unzipping process, otherwise COM is used.
    #>
    [CmdletBinding()]
    param (
    ...
    if (($PSVersionTable.PSVersion.Major -ge 3) -and
       ((Get-ItemProperty -Path "HKLM:\Software\Microsoft\NET Framework Setup\NDP\v4\Full" -ErrorAction SilentlyContinue).Version -like "4.6*" -or
       (Get-ItemProperty -Path "HKLM:\Software\Microsoft\NET Framework Setup\NDP\v4\Client" -ErrorAction SilentlyContinue).Version -like "4.6*")) {
